...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	query := fmt.Sprintf(`
		...
		AND (genres @> $2 OR $2 = '{}')
		ORDER BY %s %s, id ASC
		LIMIT $3 OFFSET $4`, filters.sortColumn(), filters.sortDescription())
...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(movie.Year >= 1888, "year", "must be greater 1888")
	...
	v.Check(len(movie.Genres) >= 1, "genres", "must be a positive number")
